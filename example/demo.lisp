...
(defun create-synth-3 (voices-num)
  ...
  (defun noteoff (voice &rest rest)
    (apply (getf voice :noteoff) rest))
  (let* ((input (corn.node:make-input :channels 2))
         (filter (create-biquad-filter :channels 2 :type :low-pass))
         (voices (loop
                   repeat voices-num
                   collect (make-voice-3 (biquad-filter-input filter))))
         (voice-manager (create-voice-manager :nodes voices
                                              :noteon #'noteon
                                              :noteoff #'noteoff)))
    (build (filter
            :frequency ((make-param :value 2000))
            :gain ((make-param :value 1))
            :q ((make-param :value 1)))
           input)
    (make-synth :channels 2
                :input input
                :voice-manager voice-manager)))
...
(let* ((time (+ (current-time) 0.1))
  ...
  (flet ((note-1 (d notenum)
           ...
           (synth-noteoff synth-1 :time (- (incf time-1 (* d beat)) 0.05) :notenum notenum))
         (note-2 (notenum)
    ...
    ; outro
    (dotimes (i 2)
      (dolist (n (list (n 4 c) (n 4 e) (n 4 g) (n 4 e)))
        (note-2 n)))
    (synth-noteon synth-2 :time time-2 :notenum (n 4 c))
    (synth-noteoff synth-2 :time (+ time-2 (* 4 beat)) :notenum (n 4 c))
    (synth-noteon synth-2 :time (+ time-2 0.04) :notenum (n 4 e))
    (synth-noteoff synth-2 :time (+ time-2 (* 4 beat)) :notenum (n 4 e))
    (synth-noteon synth-2 :time (+ time-2 0.08) :notenum (n 4 g))
    (synth-noteoff synth-2 :time (+ time-2 (* 4 beat)) :notenum (n 4 g))
    (note-1 7 (n 4 d))
    (incf time-3 (* 1 beat))
